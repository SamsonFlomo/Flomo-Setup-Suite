...
function OrganizationForm({ editingOrganization, clearEdit }) {
  ...
  function handleSubmit(event) {
    ...
    if (editingOrganization) {
      const updatedOrganization = {
        ...
        departments: editingOrganization.departments || [],
  ...
  return (
    <form onSubmit={handleSubmit}>
      <h3>{editingOrganization ? "Edit Organization" : "Add Organization"}</h3>
      ...
      <button type="submit">
        {editingOrganization ? "Save Changes" : "Add Organization"}
      </button>
    </form>
  );
}
